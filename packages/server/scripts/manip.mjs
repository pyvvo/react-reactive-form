...
const manip = async () => {
  const data = industries
    .filter((val) => val.code != "category")
    .map((value, index, array) => {
      const categoryName = industries.find((val) => val.code === "category" && val.section === value.section).name;
  ...
  console.log(data);
  fs.writeFile("./result.js", JSON.stringify(data));
};
